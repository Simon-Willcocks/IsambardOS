...
void __attribute__(( noreturn, noinline, optimize( "-Os" ) )) el3_enter_thread( thread_switch threads )
{
...
void __attribute__(( noreturn, noinline, optimize( "-Os" ) )) el3_resume_thread( thread_context *thread )
{
  //asm volatile ( ".if 
  asm volatile ( "ldp x2, x3, [%[thread], #%[offset]]"
             "\n\tmsr elr_el3, x2"
...
// Support routines
// Don't make any calls from this routine, we don't want a stack frame
// This routine is used at secure el2, to enter other secure threads at el0
void __attribute__(( noreturn, noinline, optimize( "-Os" ) )) el2_enter_thread( thread_switch threads )
{
...
// Don't make any calls from this routine, we don't want a stack frame
// Re-load AAPCS64 - corruptable registers
void __attribute__(( noreturn, noinline, optimize( "-Os" ) )) el2_resume_thread( thread_context *thread )
{
  //asm volatile ( ".if 
  asm volatile ( "ldp x2, x3, [%[thread], #%[offset]]"
             "\n\tmsr elr_el2, x2"
