...
void c_bsod()
{
...

  {
    ...
    if (!started && !done)
    {
      ...
      set_system_reg( sp_el1, 0x300000 );
      set_system_reg( sp_el0, 0x400000 );
      modify_system_reg( sctlr_el1, 0x4005005, 0x4005005 ); // Set UCI (don't trap EL0 DC cache instructions), DZE (ditto, ZVA), I, C, M - Stops it from returning, but not illegal mode
      modify_system_reg( scr_el3, 1, 1 ); // Set NS bit
      modify_system_reg( hcr_el2, (1 << 31), (0 << 31) ); // Clear RW (32-bit)
      done = 1; started = 1;
    }
